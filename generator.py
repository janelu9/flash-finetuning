...
class Generator:
    '''
    generator = Generator(llama)
    inp = torch.randint(3, 49960, (2, 32)).cuda()
    out1 = generator.sample(inp, max_length = 2048)
    out2 = generator.beam_search(inp, max_length = 2048)
    '''
    def __init__(self, CausalLM ):
    ...
    def beam_search(self,input_ids,max_length =1024,num_beams =15):
        ...
        def prepare(input_ids):
            out,cache= self.CausalLM.model(input_ids,use_cache = True,return_dict =False)
            logits = self.CausalLM.lm_head(out[:,-1,:])
            log_probs = F.log_softmax(logits,dim=-1)
            cur_log_probs,cur_word = log_probs.topk(num_beams)
            cur_seqs = cur_word.clone().unsqueeze(-1)
            end = cur_word == self.CausalLM.config.eos_token_id
            end_flags = end.float()
            end_log_probs = torch.full_like(cur_log_probs,-self.INF)
            end_word = torch.full_like(cur_seqs,self.CausalLM.config.eos_token_id)
            end_seqs = end_word.clone()
            end_log_probs_pre = end_log_probs.clone()
            end_flags_pre = torch.zeros_like(end_flags)
            if torch.any(end):
                end_log_probs = cur_log_probs+(1.-end_flags)*-self.INF
                cur_log_probs += end_flags*-self.INF
            ...
            vocab_size = self.CausalLM.model.vocab_size
    ...
    #         batch_pos_ = batch_pos.tile(1,num_beams)
        ...
        def stop(cur_log_probs,end_log_probs_pre,end_flags_pre):
            ...
            min_end_log_probs += (1. - end_flags_pre.max(1)[0]) * -self.INF   
            return torch.all(min_end_log_probs>max_cur_log_probs)
        
        def step(cur_word,position_id,cache,cur_log_probs,end_seqs):        
            out,cache= self.CausalLM.model(cur_word.contiguous().view(-1,1),position_ids = position_id,past_key_values = cache,use_cache = True,return_dict =False)
            logits = self.CausalLM.lm_head(out[:,-1,:])
            log_probs = F.log_softmax(logits,dim=-1)
    ...
    #         cur_log_probs,topk_ids = cur_log_probs.view(batch_size,-1).topk(num_beams)
        ...
        def update(end_log_probs,end_flags,end,end_seqs,cur_seqs,beam_coordinate,cur_word,cur_log_probs,cache):
            ...
            combine_log_probs =torch.cat([end_log_probs, cur_log_probs+(1.-cur_end_flags)*-self.INF],1)
            ...
            cur_log_probs += cur_end_flags *-self.INF
    ...
    def sample(self, input_ids, max_length = 1024, num_beams = 1, top_k = 50, top_p = 1, T = 1):
        ...
        def prepare(input_ids):
            ...
            cur_topp_probs = (cur_topk_probs.masked_fill_(torch.cat([left_pad,cur_topk_probs.cumsum(-1)[:,:-1] > top_p],-1),-self.INF)/T).softmax(-1)
